...
+ (UIImage *)imageName:(NSString *)imageName bundlePath:(NSString *)path {
    return [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"%@/%@.png",path,imageName]];
}
...
+ (UIImage *)qrImageForString:(NSString *)string imageSize:(CGFloat)Imagesize logoImageSize:(CGFloat)waterImagesize {
    ...
    [filter setValue:@"L" forKey:@"inputCorrectionLevel"];//设置二维码的纠错水平，越高纠错水平越高，可以污损的范围越大
...
+ (NSString *)saveImage:(UIImage *)image {
    ...
    NSString *dataFilePath = [cacheDir stringByAppendingPathComponent:@"draw"];
    BOOL isDir = NO;
    BOOL existed = [[NSFileManager defaultManager] fileExistsAtPath:dataFilePath isDirectory:&isDir];
    if (!(isDir && existed)) {
    ...
    NSString *savedImagePath=[dataFilePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@sign.JPG",str]];
...
+ (UIImage *)getImageFromView:(UIView *)theView landscapeLeft:(BOOL)landscapeLeft; {
    ...
    UIGraphicsEndImageContext();
    if (landscapeLeft) {
        return [image rotate:UIImageOrientationLeft];
    } else {
        return image;
    }
}
